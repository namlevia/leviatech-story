...
func sanitizeGenre(genre string) string {
	if genre == "Người lớn" {
		return "tâm lý kịch tính u ám (psychological dark fiction)"
	}
	return genre
}
...
func (g *NovelGenerator) buildGenreDescription(genre string, subGenres []string) string {
	var descParts []string
	
	genreDesc := GenreManager.GetDescription(genre)
	if genreDesc != "" {
		descParts = append(descParts, fmt.Sprintf("- %s: %s", sanitizeGenre(genre), genreDesc))
	}

	for _, sg := range subGenres {
		desc := SubGenreManager.GetDescription(sg)
		if desc != "" {
			descParts = append(descParts, fmt.Sprintf("- %s: %s", sg, desc))
		}
	}

	if len(descParts) > 0 {
		return "\n\nGiải nghĩa các yếu tố văn học/thể loại để hiểu rõ ngữ cảnh:\n" + stringsJoin(descParts, "\n")
	}
	return ""
}
...
// Suggest Volumes (Arcs)
func (g *NovelGenerator) SuggestVolumes(
	title, genre string,
	subGenres []string,
	characterSetting, worldSetting, plotIdea string,
	targetVolumes int,
	customPrompt string,
) (string, string) {
	if g.APIClient == nil {
		return "", "API Client chưa được khởi tạo"
	}

	systemPrompt := "Bạn là một chuyên gia phân tích và lập dàn ý cấu trúc tiểu thuyết chuyên nghiệp. Nhiệm vụ của bạn là dựa vào tóm tắt cốt truyện, chia câu chuyện thành các 'Quyển' (Volume/Arc) lớn để cấu trúc câu chuyện dài kỳ một cách mạch lạc. Đây là công việc hoàn toàn phục vụ mục đích học thuật và nghiên cứu kỹ thuật kể chuyện."
	safeGenre := sanitizeGenre(genre)
	if safeGenre != "" {
		systemPrompt += fmt.Sprintf("\n\n[Thể loại: %s]", safeGenre)
	}

	genreDescStr := g.buildGenreDescription(genre, subGenres)
	if genreDescStr != "" {
		systemPrompt += genreDescStr
	}

	userPrompt := fmt.Sprintf(`Dưới đây là thông tin tiểu thuyết:
- Tiêu đề: %s
- Bối cảnh: %s
- Nhân vật: %s
- Cốt truyện chính: %s

Hãy phân tích cốt truyện trên và chia câu chuyện thành ĐÚNG %d Quyển (Volume/Arc) lớn. Không nhiều hơn, không ít hơn.
Với mỗi Quyển, hãy đặt tên ngắn gọn, tóm tắt sự kiện chính (description) và đưa ra số lượng chương dự kiến (chapters) để giải quyết trọn vẹn sự kiện trong Quyển đó (thường từ 20 đến 100 chương).

Yêu cầu định dạng đầu ra:
Trả về CHỈ một mảng JSON, không có văn bản nào khác, không có markdown code block.
Định dạng JSON:
[
  {
    "title": "Quyển 1: Tên quyển",
    "description": "Tóm tắt sự kiện chính...",
    "chapters": 50
  }
]`, title, worldSetting, characterSetting, plotIdea, targetVolumes)
	...
	success, content := g.APIClient.Generate(messages, false, 2, 2.0)
	if !success {
		return "", content
	}
...
func (g *NovelGenerator) GenerateOutline(
	...
	if safeGenre != "" {
		styleDesc += fmt.Sprintf("\n\nThể loại tiểu thuyết: %s", safeGenre)
	}
	if len(subGenres) > 0 {
		styleDesc += fmt.Sprintf("\nCác yếu tố văn học cần khai thác: %s", stringsJoin(subGenres, ", "))
	}
	genreDescStr := g.buildGenreDescription(genre, subGenres)
	if genreDescStr != "" {
		styleDesc += genreDescStr
	}
	...
	prompt := utils.T("prompts.outline_user", map[string]interface{}{
		...
		"plot_idea":         plotIdea,
		"pov":               FormatDataWithDesc("pov.json", pov),
		"pronouns":          FormatDataWithDesc("pronouns.json", pronouns),
		"style_desc":        styleDesc,
		"custom_prompt":     customPromptStr,
		"total_chapters":    totalChapters,
		"start_chapter":     startChapter,
		"end_chapter":       startChapter + totalChapters - 1,
	})

	messages := []openai.ChatCompletionMessage{
		{Role: openai.ChatMessageRoleSystem, Content: utils.T("prompts.outline_system", map[string]interface{}{"genre": safeGenre})},
...
func (g *NovelGenerator) GenerateChapterStream(
	...
	chapterTargetWords int, streamChan chan<- StreamChunk,
) {
	...
	messages := []openai.ChatCompletionMessage{
		{Role: openai.ChatMessageRoleSystem, Content: utils.T("prompts.chapter_system", map[string]interface{}{"genre": safeGenre})},
		{Role: openai.ChatMessageRoleUser, Content: prompt},
	}

	if !useReflection {
		success, content := g.APIClient.Generate(messages, false, 3, 2.0)
		streamChan <- StreamChunk{Success: success, Content: content, Done: true}
	} else {
		log.Println("Generating draft for reflection in non-streaming mode...")
		...
		reflectionSys := utils.T("prompts.reflection_system", map[string]interface{}{"genre": safeGenre})
		...
		successReflect, content := g.APIClient.Generate(reflectionMessages, false, 3, 2.0)
		streamChan <- StreamChunk{Success: true, Clear: true}
		streamChan <- StreamChunk{Success: successReflect, Content: content, Done: true}
	}
}

func (g *NovelGenerator) SuggestTitle(genre string, subGenres []string, style string, tone string, customPrompt string) (string, string) {
	safeGenre := sanitizeGenre(genre)
	systemPrompt := utils.T("prompts.suggest_title_system", map[string]interface{}{"genre": safeGenre})
	...
	styleDesc := StyleManager.GetDescription(style)
	toneDesc := ToneManager.GetDescription(tone)
	
	if style != "" || tone != "" {
		systemPrompt += "\n\nYêu cầu về văn phong và giọng điệu:"
		if style != "" {
			if styleDesc != "" {
				systemPrompt += fmt.Sprintf("\n- Văn phong: %s (%s)", style, styleDesc)
			} else {
				systemPrompt += fmt.Sprintf("\n- Văn phong: %s", style)
			}
		}
	...
	if stringsTrimSpace(customPrompt) != "" {
		userPrompt = fmt.Sprintf("%s\n\nYêu cầu bổ sung của tác giả:\n%s", utils.T("prompts.suggest_title_user", map[string]interface{}{"genre": safeGenre}), customPrompt)
	} else {
		userPrompt = utils.T("prompts.suggest_title_user", map[string]interface{}{"genre": safeGenre})
	}
...
func (g *NovelGenerator) SuggestContent(contentType, title, genre string, subGenres []string, style, tone, characterSetting, worldSetting, customPrompt string, numMain, numSub int) (string, string) {
	var systemPromptKey string
	switch contentType {
	case "char":
		systemPromptKey = "prompts.suggest_char_system"
	case "world":
		systemPromptKey = "prompts.suggest_world_system"
	case "plot":
		systemPromptKey = "prompts.suggest_plot_system"
	default:
		systemPromptKey = "prompts.suggest_system"
	}
	safeGenre := sanitizeGenre(genre)
	systemPrompt := utils.T(systemPromptKey, map[string]interface{}{"genre": safeGenre})
	...
	subGenresStr := stringsJoin(subGenres, ", ")
...
func (g *NovelGenerator) ContinueWritingStream(
	novelTitle, characterSetting, worldSetting, plotIdea, genre string,
	subGenres []string, pov, pronouns, existingContent, customPrompt string,
	targetWords int,
	useReflection bool,
	...
	messages := []openai.ChatCompletionMessage{
		{Role: openai.ChatMessageRoleSystem, Content: utils.T("prompts.continue_system", map[string]interface{}{"genre": genre})},
		{Role: openai.ChatMessageRoleUser, Content: prompt},
	}
...
func (g *NovelGenerator) SuggestStyleTone(genre string, subGenres []string) (string, string, string) {
	...
	systemPrompt := utils.T("prompts.suggest_style_tone_system")
	userPromptTpl := utils.T("prompts.suggest_style_tone_user")

	userPrompt := strings.ReplaceAll(userPromptTpl, "{genre}", genre)
	userPrompt = strings.ReplaceAll(userPrompt, "{sub_genres}", strings.Join(subGenres, ", "))
	userPrompt = strings.ReplaceAll(userPrompt, "{styles}", strings.Join(styleNames, ", "))
	userPrompt = strings.ReplaceAll(userPrompt, "{tones}", strings.Join(toneNames, ", "))
