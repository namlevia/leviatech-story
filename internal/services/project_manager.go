...
func (pm *ProjectManager) CreateProject(title, genre string, subGenres []string, pov, pronouns, charSetting, worldSetting, plotIdea string, volumes []core.Volume) (*core.NovelProject, error) {
...
func (pm *ProjectManager) SaveProject(p *core.NovelProject) error {
	...
	volJSON, _ := json.Marshal(p.Volumes)
	if string(volJSON) == "null" {
		volJSON = []byte("[]")
	}
	log.Printf("DEBUG SaveProject: ID=%s, volJSON=%s", p.ID, string(volJSON))

	_, err = tx.Exec(`INSERT INTO projects 
		(id, title, genre, sub_genres, pov, pronouns, character_setting, world_setting, plot_idea, volumes, created_at, updated_at) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		p.ID, p.Title, p.Genre, string(sgJSON), p.Pov, p.Pronouns, p.CharacterSetting, p.WorldSetting, p.PlotIdea, string(volJSON), p.CreatedAt, p.UpdatedAt)
...
func (pm *ProjectManager) LoadProject(id string) (*core.NovelProject, error) {
	...
	var sgStr, volStr string
	err := db.QueryRow("SELECT id, title, genre, sub_genres, pov, pronouns, character_setting, world_setting, plot_idea, volumes, created_at, updated_at FROM projects WHERE id = ?", id).
		Scan(&p.ID, &p.Title, &p.Genre, &sgStr, &p.Pov, &p.Pronouns, &p.CharacterSetting, &p.WorldSetting, &p.PlotIdea, &volStr, &p.CreatedAt, &p.UpdatedAt)
	...
	if volStr != "" {
		json.Unmarshal([]byte(volStr), &p.Volumes)
	}
