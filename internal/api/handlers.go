...
func UpdateProject(c *fiber.Ctx) error {
	...
	body := c.Body()
	fmt.Println("UpdateProject body:", string(body))
	if err := c.BodyParser(&p); err != nil {
		return c.Status(400).JSON(fiber.Map{"error": "Invalid request body"})
	}
	fmt.Printf("Parsed volumes: %+v\n", p.Volumes)
...
func CreateProject(c *fiber.Ctx) error {
	type Request struct {
		...
		Chapters         []core.Chapter `json:"chapters"`
	}
	...
	pm := services.NewProjectManager()
	project, err := pm.CreateProject(req.Title, req.Genre, req.SubGenres, req.Pov, req.Pronouns, req.CharacterSetting, req.WorldSetting, req.PlotIdea, req.Volumes)
	if err != nil {
	...
	if err := pm.SaveProject(project); err != nil {
		fmt.Println("SaveProject Error:", err)
		return c.Status(500).JSON(fiber.Map{"error": err.Error()})
	}
