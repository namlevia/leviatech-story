...
export const api = {
  ...
  async generateOutline(data: OutlineReq) {
    ...
    if (json.outline) {
      json.outline = json.outline.replace(/<think>[\s\S]*?<\/think>\n*/g, '').replace(/<\/?think>\n*/g, '').trimStart();
    }
    return json;
  },

  async appendOutline(projectId: string, data: { total_chapters: number; custom_outline_prompt: string }) {
    const res = await fetch(`${API_BASE_URL}/projects/${projectId}/append-outline`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
    if (!res.ok) throw new Error("Failed to append outline");
    return res.json();
  },

  async suggestVolumes(data: {
    title: string;
    genre: string;
    sub_genres: string[];
    character_setting: string;
    ...
    const res = await fetch(`${API_BASE_URL}/generator/suggest-volumes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    });
    if (!res.ok) {
      ...
      throw new Error(errData?.error || "Failed to suggest volumes");
    }
    return res.json();
  },

  async suggestTitle(data: { genre: string; sub_genres: string[]; style?: string; tone?: string; custom_prompt?: string }) {
    ...
    const json = await res.json();
    if (json.titles) {
      json.titles = json.titles.replace(/<think>[\s\S]*?<\/think>\n*/g, '').replace(/<\/?think>\n*/g, '').trimStart();
    }
    return json;
  },
  ...
  async suggestContent(data: { 
    ...
    const json = await res.json();
    if (json.content) {
      json.content = json.content.replace(/<think>[\s\S]*?<\/think>\n*/g, '').replace(/<\/?think>\n*/g, '').trimStart();
    }
    return json;
  },
  ...
  async getDataItems(type: 'genres' | 'sub_genres' | 'styles' | 'tones' | 'pov' | 'pronouns' | 'story_structures') {
    const res = await fetch(`${API_BASE_URL}/data/${type}`);
    if (!res.ok) throw new Error(`Failed to fetch ${type}`);
    return res.json();
  },

  async addDataItem(type: 'genres' | 'sub_genres' | 'styles' | 'tones' | 'pov' | 'pronouns' | 'story_structures', data: any) {
    const res = await fetch(`${API_BASE_URL}/data/${type}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
  ...
  async updateDataItem(type: 'genres' | 'sub_genres' | 'styles' | 'tones' | 'pov' | 'pronouns' | 'story_structures', name: string, data: any) {
  ...
  async deleteDataItem(type: 'genres' | 'sub_genres' | 'styles' | 'tones' | 'pov' | 'pronouns' | 'story_structures', name: string) {
    const res = await fetch(`${API_BASE_URL}/data/${type}/delete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
