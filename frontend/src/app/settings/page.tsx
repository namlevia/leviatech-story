...
type Tab = "general" | "backends" | "genres" | "sub_genres" | "styles" | "tones" | "pov" | "pronouns" | "prompts" | "story_structures";
...
export default function SettingsPage() {
  ...
  const loadData = async (silent = false) => {
    ...
    try {
      const [bData, cfgData, gData, sgData, stData, tnData, povData, proData, ssData, pData] = await Promise.all([
  ...
  const tabs = [
    ...
    { id: "story_structures", label: "Cấu Trúc Truyện", icon: Book },
        ...
        <div className="w-full h-full">
          {loading ? (
            ...
            <div className="min-h-full">
              ...
              {activeTab === "story_structures" && <DataConfig type="story_structures" title="Cấu Trúc Truyện" data={storyStructures} refresh={loadData} />}
              ...
              {activeTab === "prompts" && <PromptsConfig prompts={prompts} genres={genres} refresh={loadData} />}
            </div>
          )}
        </div>
      </div>
...
function GeneralConfig({ config, styles, tones, povs, pronouns, refresh }: { config: any, styles: any[], tones: any[], povs: any[], pronouns: any[], refresh: (silent?: boolean) => void }) {
  const [form, setForm] = useState(config || {
    ...
    character_development: "Trung bình",
    plot_complexity: "Trung bình"
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
        ...
        <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
          <label className="text-sm font-medium text-text-muted">Mức độ phát triển nhân vật</label>
          <select
            value={form.character_development}
            onChange={e => setForm({...form, character_development: e.target.value})}
            className="select w-full"
          >
            <option value="Chi tiết" className="bg-zinc-900 text-text-main">Chi tiết</option>
            <option value="Trung bình" className="bg-zinc-900 text-text-main">Trung bình</option>
            <option value="Tóm tắt" className="bg-zinc-900 text-text-main">Tóm tắt</option>
          </select>
        </div>
        <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
          <label className="text-sm font-medium text-text-muted">Độ phức tạp cốt truyện</label>
          <select
            value={form.plot_complexity}
            onChange={e => setForm({...form, plot_complexity: e.target.value})}
            className="select w-full"
          >
            <option value="Rất phức tạp" className="bg-zinc-900 text-text-main">Rất phức tạp</option>
            <option value="Phức tạp" className="bg-zinc-900 text-text-main">Phức tạp</option>
            <option value="Trung bình" className="bg-zinc-900 text-text-main">Trung bình</option>
            <option value="Đơn giản" className="bg-zinc-900 text-text-main">Đơn giản</option>
          </select>
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-border-soft pt-6">
        <button 
          ...
          className="flex items-center gap-2 text-text-main font-bold hover:text-primary transition-colors"
        >
          Thông số AI nâng cao
          <span className="text-text-muted">{showAdvanced ? "▲" : "▼"}</span>
        </button>
        {showAdvanced && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-4 animate-in fade-in slide-in-from-top-2">
            <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
              <label className="text-sm font-medium text-text-muted flex justify-between items-center">
                Top P
              </label>
              <input 
                type="number"
              ...
              <div className="text-[10px] text-text-subtle leading-tight">
                Giới hạn các từ có tổng xác suất bằng P. Giá trị nhỏ giúp ổn định văn phong.
              </div>
            </div>
            <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
              <label className="text-sm font-medium text-text-muted flex justify-between items-center">
              ...
              />
              <div className="text-[10px] text-text-subtle leading-tight">
                Chỉ giữ K từ có xác suất cao nhất. Giá trị nhỏ làm AI bớt "ảo giác".
              </div>
            </div>
            <div className="space-y-2 p-3 bg-bg-input rounded-xl border border-border-soft">
              ...
              />
              <div className="text-[10px] text-text-subtle leading-tight">
                Giới hạn độ dài tối đa của phản hồi (tính bằng token).
              </div>
            </div>
            {/* Cột thứ 4 trống để cân đối, hoặc có thể thêm 1 ô rỗng hoặc thông tin khác */}
            <div className="space-y-2 p-3 bg-bg-panel/30 rounded-xl border border-dashed border-border-soft flex items-center justify-center">
               <span className="text-xs text-text-subtle italic text-center px-2">Cấu hình nâng cao<br/>chỉ dành cho người có kinh nghiệm</span>
            </div>
          </div>
        )}
      </div>
...
function DataConfig({ type, title, data, refresh }: { type: 'genres' | 'sub_genres' | 'styles' | 'tones' | 'pov' | 'pronouns' | 'story_structures', title: string, data: any[], refresh: () => void }) {
...
function PromptsConfig({ prompts, genres, refresh }: { prompts: any, genres: any[], refresh: (silent?: boolean) => void }) {
  const [form, setForm] = useState<any>({});
  const [selectedGenre, setSelectedGenre] = useState("Mặc định");
  const [saving, setSaving] = useState(false);
  const [saveSuccess, setSaveSuccess] = useState(false);
  ...
  useEffect(() => {
    if (prompts) {
      // Ensure the form has the new structure { [genre]: { key: value } }
      const newForm = { ...prompts };
      if (!newForm["Mặc định"]) {
        newForm["Mặc định"] = {};
      }
      setForm(newForm);
  ...
  const fields = [
    { key: "suggest_title_system", label: "Prompt Gợi ý Tên Truyện" },
    { key: "suggest_char_system", label: "Prompt Gợi ý Nhân Vật" },
    { key: "suggest_world_system", label: "Prompt Gợi ý Bối Cảnh" },
    { key: "suggest_plot_system", label: "Prompt Gợi ý Cốt Truyện" },
    ...
    { key: "suggest_style_tone_system", label: "Prompt Văn phong - Giọng điệu" }
  ];
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:justify-start gap-3 mb-6">
        <label className="text-sm font-medium text-text-muted whitespace-nowrap text-center sm:text-left w-full sm:w-auto">Chọn Thể Loại:</label>
        <select 
          value={selectedGenre}
          onChange={(e) => setSelectedGenre(e.target.value)}
          className="input w-full sm:!w-auto max-w-full bg-bg-input border-border-soft"
        >
          <option value="Mặc định">⭐ Mặc định (Dùng chung)</option>
      ...
      <div className="space-y-2 p-4 bg-bg-input rounded-xl border border-border-soft">
        <textarea 
          value={form[selectedGenre]?.[activeField] || ""} 
          onChange={e => {
            setForm({
              ...form,
              [selectedGenre]: {
          ...
          className="w-full bg-bg-main border border-border-soft rounded-lg px-4 py-3 text-sm text-text-main focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all resize-y min-h-[300px] custom-scrollbar font-mono"
          placeholder={selectedGenre !== "Mặc định" ? `Nếu để trống, sẽ dùng tự động dùng chung prompt từ mục "Mặc định"...` : "Nhập nội dung prompt ở đây..."}
        />
      </div>
    </div>
  );
