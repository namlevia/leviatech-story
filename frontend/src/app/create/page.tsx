...
import { Sparkles, Loader2, Play, ChevronDown, ChevronUp, StopCircle, Wand2, Info, Search, X, Users, FileText, PlayCircle, Globe, Lightbulb, FolderOpen, Copy, Lock, CheckCircle2 } from "lucide-react";
...
export default function CreatePage() {
  ...
  const [storyStructures, setStoryStructures] = useState<any[]>([]);
  ...
  const [chapterTargetWords, setChapterTargetWords] = useState<number>(4000);
  ...
  // Modal State
  const [modalType, setModalType] = useState<"genre" | "sub_genre" | "style" | "tone" | "pov" | "pronouns" | "story_structure" | null>(null);
  ...
  const [volumeOutlines, setVolumeOutlines] = useState<Record<number, string>>({});
  ...
  const [targetVolumesCount, setTargetVolumesCount] = useState<number>(4);
  const [suggestingVolumes, setSuggestingVolumes] = useState(false);
  const [volumeStructurePrompt, setVolumeStructurePrompt] = useState("");
  ...
  const [completedVolumes, setCompletedVolumes] = useState<number[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getDataItems('story_structures').catch(() => []),
      api.getGenerationConfig().catch(() => null)
    ]).then(([g, sg, st, tn, povData, proData, ssData, genCfg]) => {
      setGenres(g);
      ...
      setStoryStructures(ssData);
      if (ssData.length > 0) setStoryStructure(ssData[0].name);

      if (genCfg && genCfg.chapter_target_words) {
        setChapterTargetWords(genCfg.chapter_target_words);
      }
    });
  }, []);
    ...
    try {
      ...
      if (p.plot_idea) setPlotIdea(p.plot_idea);
        if (p.volumes) {
          ...
          setTargetVolumesCount(p.volumes.length);
        }
        if (p.chapters && p.chapters.length > 0) {
          ...
          // Calculate completedVolumes
          if (p.volumes) {
            let currentChCount = 0;
            let compVols: number[] = [];
            for (let i = 0; i < p.volumes.length; i++) {
              let expected = p.volumes[i].chapters || p.volumes[i].estimated_chapters || 20;
              currentChCount += expected;
              if (p.chapters.length >= currentChCount) {
                compVols.push(i);
              } else {
                break;
              }
            }
            setCompletedVolumes(compVols);
            if (compVols.length < p.volumes.length) {
              setSelectedVolumeIdx(compVols.length);
              setTotalChapters(p.volumes[compVols.length].chapters || p.volumes[compVols.length].estimated_chapters || 20);
            } else {
              setSelectedVolumeIdx(p.volumes.length > 0 ? p.volumes.length - 1 : null);
            }
          }
          
          setOutline(""); // Don't populate outline textarea with existing chapters
        }
  ...
  const handleSuggestContent = async (type: 'char' | 'world' | 'plot') => {
    ...
    try {
      const res = await api.suggestContent({
        content_type: type,
        title, genre, sub_genres: selectedSubGenres, style, tone,
        character_setting: characterSetting,
        world_setting: worldSetting,
        custom_prompt: type === 'char' ? charPrompt : 
                       type === 'world' ? worldPrompt : 
                       (storyStructure !== "Không chọn — tự do" ? `Sử dụng cấu trúc truyện: ${storyStructure}. ${plotPrompt}` : plotPrompt),
  ...
  const handleSuggestVolumes = async () => {
    if (!title) return toast("Vui lòng nhập tên truyện", "error");
    setSuggestingVolumes(true);
    try {
      const res = await api.suggestVolumes({
        title, genre, sub_genres: selectedSubGenres,
        character_setting: characterSetting, world_setting: worldSetting, plot_idea: plotIdea,
        target_volumes: targetVolumesCount,
        custom_outline_prompt: volumeStructurePrompt
      });
      if (res.volumes && res.volumes.length > 0) {
        setVolumes(res.volumes);
        setSelectedVolumeIdx(0);
        setTotalChapters(res.volumes[0].chapters || res.volumes[0].estimated_chapters || 20);
        ...
        try {
          ...
          if (!projectId) {
            const pRes = await api.createProject(payload);
            setProjectId(pRes.id);
          } else {
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      if (volumes.length > 0 && selectedVolumeIdx !== null) {
         startChapter = 1;
         for (let i = 0; i < selectedVolumeIdx; i++) {
            startChapter += (volumes[i].chapters || volumes[i].estimated_chapters || 20);
         }
         endChapter = startChapter + totalChapters - 1;
      }
      ...
      setVolumeOutlines(prev => ({ ...prev, [selectedVolumeIdx as number]: res.outline }));
      toast("Đã sinh dàn ý Quyển thành công!", "success");
    } catch (err) {
  ...
  const handleUndoVolume = async () => {
    if (selectedVolumeIdx === null || !completedVolumes.includes(selectedVolumeIdx)) return;
    if (completedVolumes[completedVolumes.length - 1] !== selectedVolumeIdx) {
      return toast("Chỉ có thể làm lại Quyển vừa thêm gần nhất. Vui lòng làm lại quyển sau trước.", "error");
    }

    const newChapters = chapters.filter(ch => ch.volumeIdx !== selectedVolumeIdx);
    setChapters(newChapters);
    
    const newCompleted = completedVolumes.filter(v => v !== selectedVolumeIdx);
    setCompletedVolumes(newCompleted);
    ...
    try {
      ...
      if (projectId) await api.updateProject(projectId, payload);
    } catch (e) {
      console.error("Auto-save failed:", e);
    }
    toast("Đã hủy dàn ý quyển này. Bạn có thể sinh lại.", "success");
  };
  ...
  const handleManualParseOutline = async () => {
    try {
      ...
      if (parseRes.chapters) {
        let maxNum = 0;
        chapters.forEach((ch: any) => { if (ch.num > maxNum) maxNum = ch.num; });
        const newChapters = parseRes.chapters.map((ch: any) => {
          maxNum++;
          return { ...ch, num: maxNum, volumeIdx: selectedVolumeIdx };
        });
        const allChapters = [...chapters, ...newChapters];
        
        setChapters(allChapters);
        
        let nextCompletedVolumes = completedVolumes;
        if (selectedVolumeIdx !== null && !completedVolumes.includes(selectedVolumeIdx)) {
          nextCompletedVolumes = [...completedVolumes, selectedVolumeIdx];
          setCompletedVolumes(nextCompletedVolumes);
        }
        ...
        // Auto select next incomplete volume
        let nextIdx = null;
        for (let i = 0; i < volumes.length; i++) {
          if (!nextCompletedVolumes.includes(i)) {
            nextIdx = i;
            break;
          }
        }
        if (nextIdx !== null) {
          ...
          setTotalChapters(volumes[nextIdx].chapters || volumes[nextIdx].estimated_chapters || 20);
          setOutline(volumeOutlines[nextIdx] || "");
        }
  ...
  const startGenerationLoop = async () => {
    ...
    for (let i = 0; i < localChapters.length; i++) {
      ...
      try {
        await new Promise<void>((resolve, reject) => {
          streamCompletion(
            ...
            (text) => {
               ...
               let displayContent = streamTextRef.current;
               const isThinking = displayContent.includes("<think>") && !displayContent.includes("</think>");
               
               displayContent = displayContent.replace(/<think>[\s\S]*?<\/think>/g, '');
               if (isThinking) {
                   displayContent = displayContent.replace(/<think>[\s\S]*/, '') + "\n\n_[AI đang phân tích và tối ưu lại bản nháp...]_";
               }
               
               setStreamContent(displayContent.trimStart());
            },
            () => resolve(),
            (err) => reject(err)
          );
        ...
        let finalCleanContent = streamTextRef.current;
        finalCleanContent = finalCleanContent.replace(/<think>[\s\S]*?<\/think>\n*/g, '');
        finalCleanContent = finalCleanContent.replace(/<\/?think>\n*/g, '');
        finalCleanContent = finalCleanContent.trimStart();
        
        localChapters[i].content = finalCleanContent;
          ...
          {activeStep === 1 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <GlassCard className="space-y-8">
                ...
                <div className="p-4 bg-bg-input border border-border-soft rounded-xl mt-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex-1 space-y-2">
                      <label className="text-sm font-medium text-text-muted">Độ dài mỗi chương (số chữ)</label>
                      <input 
                        type="number"
                        min="500"
                        max="10000"
                        ...
                        onChange={e => setChapterTargetWords(parseInt(e.target.value) || 4000)}
                        className="input w-full"
                        placeholder="(Mặc định: 4000)"
                      />
                    </div>
                    <div className="flex-1">
                      <label className="text-sm font-medium text-text-muted mb-2 block">Cài đặt nhanh</label>
                      <div className="flex flex-wrap gap-2">
                        {[1500, 2000, 3000, 4000, 5000, 6000].map(words => (
                          <button
                            key={words}
                            onClick={() => setChapterTargetWords(words)}
                            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors border ${chapterTargetWords === words ? 'bg-primary/20 text-primary border-primary/50' : 'bg-bg-panel text-text-muted border-border-soft hover:bg-white/5 hover:text-text-main'}`}
                          >
                            {words}
                          </button>
                        ))}
          ...
          {activeStep === 4 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <GlassCard className="space-y-8">
                <div className="space-y-3">
                  <div className="mb-4">
                    <label className="text-sm text-text-muted mb-2 block font-semibold uppercase tracking-wider text-center md:text-left">Cấu Trúc Truyện</label>
                    <div className="flex flex-col md:flex-row items-center gap-2 md:gap-4">
                      <select 
                        value={storyStructure} 
                        onChange={e => setStoryStructure(e.target.value)} 
                        className="w-full select appearance-none text-center md:text-left shrink-0"
                      ...
                      {storyStructure && storyStructures.find(s => s.name === storyStructure)?.description && (
                        <p className="text-xs text-brand-secondary/80 italic text-center md:text-left flex-1 animate-in fade-in slide-in-from-right-2 duration-300">
                          {storyStructures.find(s => s.name === storyStructure)?.description}
                        </p>
                      )}
                    </div>
                  </div>
          ...
          {activeStep === 5 && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <GlassCard className="space-y-6">
                {/* Volume Generator Section */}
                <div className="flex flex-col gap-4 bg-bg-panel p-5 rounded-xl border border-white/10">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-4 gap-4 md:gap-0">
                    <div className="flex justify-between items-start md:items-center w-full md:w-auto gap-2 md:gap-4 shrink-0">
                      <div>
                        <h3 className="font-bold text-white text-lg">Phân chia Quyển</h3>
                        <p className="text-sm text-text-muted mt-1">Gợi ý chia truyện thành các quyển lớn</p>
                      </div>
                      
                      <div className="hidden md:block w-px h-8 bg-white/10 mx-2"></div>

                      <button onClick={() => setShowGlobalPrompt(!showGlobalPrompt)} className="text-sm text-brand-primary hover:text-brand-hover cursor-pointer select-none inline-flex items-center gap-1 transition-colors whitespace-nowrap mt-1 md:mt-0">
                        <span className={showGlobalPrompt ? "hidden" : "inline-flex items-center gap-1"}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
                          <span className="hidden sm:inline">Thêm yêu cầu chung (Tùy chọn)</span>
                          <span className="sm:hidden">Thêm yêu cầu</span>
                        </span>
                        <span className={!showGlobalPrompt ? "hidden" : "inline-flex items-center gap-1"}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/></svg>
                          <span className="hidden sm:inline">Ẩn yêu cầu chung</span>
                          <span className="sm:hidden">Ẩn yêu cầu</span>
                        </span>
                      </button>
                    </div>

                    {selectedVolumeIdx !== null && selectedVolumeIdx >= 0 && volumes[selectedVolumeIdx] && (
                      <div className="flex-1 w-full md:w-auto flex justify-start md:justify-center px-0 md:px-4 order-last md:order-none min-w-0 mt-2 md:mt-0">
                        <div className="flex items-center gap-2 text-xs md:text-sm text-brand-secondary px-3 py-1.5 bg-brand-primary/10 rounded-lg max-w-full">
                          <span className="w-1.5 h-1.5 rounded-full bg-brand-secondary animate-pulse shrink-0"></span>
                          <span className="truncate">Đang chọn: <strong className="font-semibold">{volumes[selectedVolumeIdx].title}</strong></span>
                        </div>
                      </div>
                    )}
                    
                    <div className="flex gap-2 items-center flex-wrap w-full md:w-auto mt-2 md:mt-0 shrink-0">
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/5">
                        <span className="text-sm text-text-muted whitespace-nowrap">Số quyển:</span>
                        <input type="number" min={1} max={10} value={targetVolumesCount} onChange={(e) => setTargetVolumesCount(parseInt(e.target.value) || 4)} className="w-12 bg-transparent text-white font-bold text-center outline-none border-none"/>
                      </div>
                      <Button onClick={handleSuggestVolumes} disabled={suggestingVolumes} variant={volumes.length === 0 ? "glow" : "secondary"} className="flex-1 md:flex-none">
                        {suggestingVolumes ? <Loader2 size={16} className="animate-spin mr-2" /> : <Wand2 size={16} className="mr-2" />}
                        {volumes.length === 0 ? "Phân tích AI" : "Tạo lại cấu trúc"}
                      </Button>
                    </div>
                  </div>

                  {showGlobalPrompt && (
                    <div className="mt-4 mb-2 animate-in slide-in-from-top-2 fade-in duration-200">
                      <textarea value={volumeStructurePrompt} onChange={e => setVolumeStructurePrompt(e.target.value)} className="w-full textarea text-sm resize-none custom-scrollbar" rows={2} placeholder="Nhập yêu cầu chung cho toàn truyện... (ví dụ: bám sát hành trình phiêu lưu, kết hợp yếu tố kinh dị...)"/>
                    </div>
                  )}
                  {volumes.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                      {volumes.map((vol, idx) => {
                        const isLocked = idx > 0 && !completedVolumes.includes(idx - 1);
                        return (
                          <div key={idx} onClick={() => {
                            if (isLocked) return;
                            if (selectedVolumeIdx !== null) {
                              setVolumeOutlines(prev => ({ ...prev, [selectedVolumeIdx]: outline }));
                            }
                            setSelectedVolumeIdx(idx);
                            setTotalChapters(vol.chapters || vol.estimated_chapters || 20);
                            
                            let newOutline = volumeOutlines[idx];
                            if (!newOutline && completedVolumes.includes(idx)) {
                                let start = 0;
                                for (let i = 0; i < idx; i++) {
                                  start += (volumes[i].chapters || volumes[i].estimated_chapters || 20);
                                }
                                let expected = volumes[idx].chapters || volumes[idx].estimated_chapters || 20;
                                const volChaps = chapters.slice(start, start + expected);
                                newOutline = volChaps.map((c:any) => `Chương ${c.num}: ${c.title}\n${c.desc}`).join("\n\n");
                            }
                            setOutline(newOutline || "");
                          }} className={`p-4 rounded-xl border transition-all duration-200 ${isLocked ? 'cursor-not-allowed opacity-40 bg-white/5 border-white/5' : 'cursor-pointer'} ${!isLocked && selectedVolumeIdx === idx ? 'bg-primary-500/20 border-primary-500 shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)]' : ''} ${!isLocked && selectedVolumeIdx !== idx ? 'bg-white/5 border-white/10 hover:border-white/20' : ''}`}>
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-medium text-white flex items-center gap-2">
                                {isLocked && <Lock size={14} className="text-text-muted" />}
                                {vol.title}
                              </h4>
                              {completedVolumes.includes(idx) && <CheckCircle2 size={18} className="text-brand-primary shrink-0 ml-2" />}
                            </div>
                            <p className="text-xs text-text-muted line-clamp-3 mb-3">{vol.description}</p>
                            <div className="text-xs font-semibold text-primary-400">~ {vol.chapters || vol.estimated_chapters} chương</div>
                          </div>
                        );
                      })}
                    </div>
                ...
                {volumes.length > 0 && (
                  <div className="flex flex-col gap-4 bg-bg-panel p-5 rounded-xl border border-white/10 mt-6">

                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-2 border-b border-white/5 gap-4">
                      <button onClick={() => setShowCustomPrompt(!showCustomPrompt)} className="text-sm text-brand-primary hover:text-brand-hover cursor-pointer select-none inline-flex items-center gap-1 transition-colors whitespace-nowrap">
                        <span className={showCustomPrompt ? "hidden" : "inline-flex items-center gap-1"}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
                          <span className="hidden sm:inline">Thêm yêu cầu bổ sung cho Quyển này (Tùy chọn)</span>
                          <span className="sm:hidden">Thêm yêu cầu bổ sung</span>
                        </span>
                        <span className={!showCustomPrompt ? "hidden" : "inline-flex items-center gap-1"}>
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/></svg>
                          <span className="hidden sm:inline">Ẩn yêu cầu bổ sung</span>
                          <span className="sm:hidden">Ẩn yêu cầu bổ sung</span>
                        </span>
                      </button>

                      <div className="flex gap-4 items-center flex-wrap w-full sm:w-auto">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/5">
                          <span className="text-sm text-text-muted whitespace-nowrap">Số chương:</span>
                          <input type="number" value={totalChapters} onChange={e => setTotalChapters(parseInt(e.target.value)||20)} className="w-12 bg-transparent text-white font-bold text-center outline-none border-none" />
                        </div>
                        {selectedVolumeIdx !== null && completedVolumes.includes(selectedVolumeIdx) ? (
                          <Button variant="secondary" onClick={handleUndoVolume} className="flex-1 sm:flex-none text-red-400 hover:text-red-300 hover:bg-red-900/20 border-red-900/30">
                            <X size={16} className="mr-2 inline" /> Hủy Dàn Ý Đã Thêm
                          </Button>
                        ) : (
                          <Button variant="glow" onClick={handleGenerateOutline} disabled={generatingOutline} className="flex-1 sm:flex-none">
                            {generatingOutline ? <Loader2 size={16} className="animate-spin mr-2 inline" /> : null}
                            Sinh Dàn Ý Quyển
                          </Button>
                        )}
                      </div>
                    ...
                    {showCustomPrompt && (
                      <div className="mt-4 animate-in slide-in-from-top-2 fade-in duration-200">
                        <textarea 
                          value={customPrompt} 
                          onChange={e => setCustomPrompt(e.target.value)} 
                          className="w-full textarea text-sm resize-none custom-scrollbar" 
                          rows={2}
                          placeholder="Nhập yêu cầu riêng cho Quyển này... (Ví dụ: Nữ chính xuất hiện, tìm thấy bảo vật...)" 
                        />
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-border-soft">
                  <p className="text-sm text-text-subtle">Hệ thống sẽ nối dàn ý vào danh sách chương hiện tại của truyện.</p>
                  <div className="flex gap-2 sm:gap-4 w-full sm:w-auto">
                    <Button variant="secondary" onClick={() => setActiveStep(4)} className="flex-1 sm:flex-none px-4">Quay lại</Button>
                    <Button variant="secondary" onClick={handleManualParseOutline} disabled={!outline.trim() || (selectedVolumeIdx !== null && completedVolumes.includes(selectedVolumeIdx))} className="flex-1 sm:flex-none px-4">
                      {selectedVolumeIdx !== null && completedVolumes.includes(selectedVolumeIdx) ? "Đã Thêm Vào Truyện" : "Thêm Dàn Ý Vào Truyện"}
                    </Button>
                    <Button variant="glow" onClick={() => setActiveStep(6)} disabled={completedVolumes.length < volumes.length} className="flex-1 sm:flex-none px-4">Hoàn tất & Tiếp tục</Button>
                  </div>
                </div>
              </GlassCard>
            </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar bg-black/10">
                    {chapters.map((chapter, idx) => {
                      ...
                      if (volumes && volumes.length > 0) {
                        for (let i = 0; i < volumes.length; i++) {
                          let vChap = volumes[i].chapters || volumes[i].estimated_chapters || 20;
                          if (chapter.num <= acc + vChap) {
                              currentVolIdx = i;
                              break;
                          }
                          acc += vChap;
                      ...
                      const isFirstOfVol = chapter.num === acc + 1 && currentVolIdx !== -1;

                      return (
                        <div key={idx} className="flex flex-col gap-3">
                          {isFirstOfVol && currentVolIdx !== -1 && (
                            <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-md px-4 py-2 rounded-lg border border-primary-500/30 text-brand-primary font-bold text-sm shadow-md mt-2">
                              {volumes[currentVolIdx].title}
                            </div>
                          )}
                          <div 
                            id={`chapter-item-${idx}`}
